...
//var port = chrome.runtime.connect({name: "chrome.js"});
//
...
function startAndBurner(port){
    serial.connect(port,function(result){
                   console.log(result);
                   setTimeout(function(){
                              serialStartBurner()
                              },800)
...
function serialStartBurner()
{
    ...
    {
        var ladd = address & 0xff;
        var hadd =  (address >> 8)&0xff;
...
var hd = {
    ...
    getData:function(x){
//        
//        var p =  Math.round((this.index+1)/ x.length*100);
//        $('#progress_bar .ui-progress').animateProgress(p);
        htmlPlug.postMessage({progress:this.index, total:x.length});
    ...
    getProgress:function(p){
        var pr = (this.index+1) / x.length;
        if(this.index == x.length -1){
            pr = 1;
        }
        p(pr);
        return p;
...
function reavAndAck(info){
    
    
    
}

function testProgram(item,callback){
